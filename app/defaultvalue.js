...
var COMPONENT_SIGNATURE = `<input data-oid='{{item.id}}' class= 'fileupload' fileimage type-mode=1 ng-model='item.value' data-tempproperty='{{item.semantic.prefix}}{{item.semantic.property}}' type='file' accept='image/*' ng-required='{{item.required}}' title='Image format is required, one image only'/>
                            <div ng-repeat-start='img in item.value' class='image-line'>
                            <div class='img-frame'>
                            <img alt='' property='{{item.semantic.prefix}}{{item.semantic.property}}' src='{{img.src}}' title='{{img.name}}'>
                            </div>
                            <button ng-click='$parent.$parent.deleteImage(item,img)'>Remove</button>
                            </div>
                            <br ng-repeat-end />`
var COMPONENT_ATTACH = `<input data-oid='{{item.id}}' class= 'fileupload' fileimage type-mode=2 ng-model='item.value' data-tempproperty='{{item.semantic.prefix}}{{item.semantic.property}}' type='file' multiple accept='image/*' ng-required='{{item.required}}' title='Image format is required, at least one image is required'/>
                        <div ng-repeat-start='img in item.value' class='image-line multiple'>
                        <span>{{img.name}}</span>
                        <div class='img-frame'>
                            <img alt='' property='{{item.semantic.prefix}}{{item.semantic.property}}' src='{{img.src}}' title='{{img.name}}' >
                        </div>
